...
static char Version[]="\0$VER: audio test 0.4 (26.11.2024)\0";
...
{
    BPTR f = 0;
    struct dcb *cb[MAX_CBS] = { NULL, };
    uint32_t *buffer[MAX_CBS] = { NULL, };
    uint8_t *rbuf = NULL;
    LONG pos = 0;
    LONG readlen = 0;
    LONG filesize = 0;
    ...
    int index = -1; /* dirty trick to fill buffers one behind */
    ...
    rbuf = AllocVec (filesize, MEMF_CLEAR|MEMF_FAST);
    if (rbuf == NULL) {
    ...
    for (int i = 0; i < MAX_CBS; i++) {
        ...
        if (cb[i] == NULL) {
            fprintf(stderr, "Could not alloc mem for cb[%d]: %d\n", i, sizeof(struct dcb));
            retval = 1; 
            goto error;
        }

        buffer[i] = (uint32_t *)dma_get_mem(BUF_SIZE);
        if (buffer[i] == NULL) {
            fprintf(stderr, "Could not alloc mem for buffer[%d]: %d\n", i, BUF_SIZE);
            retval = 1; 
            goto error;
        }
        dma_setup(cb[i], (uint32_t)buffer[i], (uint32_t)HDMI_MAI_DATA_BUS, BUF_SIZE);
    }
    for (int i = 0; i < MAX_CBS; i++) {
        cb[i]->next_cb = LE32((intptr_t)cb[(i + 1) % MAX_CBS]);
        CacheClearE(cb[i], sizeof(struct dcb), CACRF_ClearD);
    }
    index = -1;
    ...
    uint32_t counter = 0;
#endif
    dma_start(ch, cb[0]); // start with empty one
    // last buffer missing
    for (int i = 0; i < filesize; i += BUF_SIZE_2) {
        uint32_t bpos2 = 0;
        if (++index >= MAX_CBS) index = 0;
        dma_wait_cb_change(ch); // wait cb change
        for (uint32_t bpos = i; bpos < i + BUF_SIZE_2; bpos += 2) {
            ULONG *b = (ULONG *)&rbuf[bpos];
            ULONG data = (ULONG)LE32(*b);

            /* result (bits): 0000 xxxx xxxx xxxx xxxx 0000 0000 0000 */
            data <<= 16;
            data >>= 4;
            data &= ~0xF;
#ifdef ADD_PARITY
            ...
            counter %= 192;
#endif
            buffer[index][bpos2++] = LE32(data);
        }
        // flush data caches
        CacheClearE(buffer[index], BUF_SIZE, CACRF_ClearD);
    };
