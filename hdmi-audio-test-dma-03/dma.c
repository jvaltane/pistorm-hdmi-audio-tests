...
#define MAX_DCB_COUNT (MAX_CBS + 1)
#define MAX_BUF_COUNT (MAX_CBS + 1)
#define PADDING_SIZE (32)

int dma_init(void)
{
    /* enough for four dcb structs + four buffers (src and dst) */
...
void dma_start (uint32_t ch, struct dcb *cb)
{
    ...
    *DMA_CH_CS(ch) = LE32(DMA_WAIT_ON_WRITES | DMA_DISDEBUG);
...
void dma_wait_cb_change (uint32_t ch)
{
    uint32_t addr = *DMA_CH_CB_ADDR(ch); /* get current cb */
    while (*DMA_CH_CB_ADDR(ch) == addr) { } /* wait until change */
}
